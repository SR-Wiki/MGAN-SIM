...
from networks import define_G, define_D, GANLoss, get_scheduler, update_learning_rate, SIMloss  # 确保导入正确
import torch.onnx
...
# Training settings
parser = argparse.ArgumentParser(description='MarkovianGAN-pytorch-implementationn')
...
parser.add_argument('--pretrained_g', type=str, default='./checkpoint/Example/netG_model_epoch_100.pth', help='Path to pretrained generator')
parser.add_argument('--pretrained_d', type=str, default='./checkpoint/Example/netD_model_epoch_100.pth', help='Path to pretrained discriminator')
...
net_g = define_G(
    ...
    opt.ngf,
    'unet_128',
    'batch',     # 确保与预训练模型使用的 norm 类型一致！
    False,       # use_dropout（根据预训练模型调整）
    'normal',
    0.02,
    gpu_ids=[0]  # GPU 配置（与预训练模型一致）
).to(device)

# 初始化判别器（net_d）
net_d = define_D(
    opt.input_nc + opt.output_nc,  # 输入通道数为真实图+生成图拼接
    opt.ndf,
    opt.discriminator,  # 确保与预训练模型使用的 netD 类型一致
    gpu_ids=[0]
).to(device)
...
print("\n===== 开始加载预训练模型 =====")
# 加载生成器预训练权重
pretrained_model_g = torch.load(opt.pretrained_g, map_location='cuda')
net_g.load_state_dict(pretrained_model_g.state_dict(), strict=True)
# 加载判别器预训练权重
pretrained_model_d = torch.load(opt.pretrained_d, map_location='cuda')
net_d.load_state_dict(pretrained_model_d.state_dict(), strict=True)
...
def freeze_encoder_layers(model):
    ...
    print(f"Froze {len(frozen_layers)} encoder down layers in UnetGenerator.")
    return frozen_layers

# 初始化生成器后，冻结编码器下采样层
freeze_encoder_layers(net_g)
...
# setup optimizer
optimizer_g = optim.Adam(net_g.parameters(), lr=opt.lr*0.1, betas=(opt.beta1, 0.999))
optimizer_d = optim.Adam(net_d.parameters(), lr=opt.lr*0.1, betas=(opt.beta1, 0.999))
...
for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):
    ...
    for batch_i, batch in enumerate(data_loader1.load_batch(train_batch_size=opt.batch_size), 1):
        ...
        # 保存测试图像（每 sample_interval 个 batch 保存一次）
